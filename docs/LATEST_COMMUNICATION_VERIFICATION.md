...
| 2026-05-03 | MELSEC iQ-R / R08CPU | `melsec:iq-r` | TCP `1025`, UDP `1027` | Device-range sample validation, all-device checker coverage, and bit-block checks. | Unsupported `S` entries are recorded in the validation note. | Retained maintainer note |
