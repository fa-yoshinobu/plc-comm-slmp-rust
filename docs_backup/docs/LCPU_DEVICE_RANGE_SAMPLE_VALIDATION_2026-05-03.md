# LCPU Device Range Sample Validation
...
- TCP port: `1025`
- UDP port: `1027`
- Rust profile: `SLMP_PLC_FAMILY=lcpu` (`Frame3E` + `Legacy`)

Profile selection:

- Do not use connection-profile probing.
- Do not select `Frame4E` for LCPU.
...
- `DeviceReadBlock` / `DeviceWriteBlock` are skipped for LCPU (`0x0406` /
  `0x1406` unsupported).
- Random read/write routes are skipped for LCPU (`0x0403` / `0x1402`
  unsupported).
- `LZ`, `RD`, and long timer/counter devices are unsupported by the LCPU catalog.
...
- TCP `1025`: `passed=245`, `read_failed=0`, `write_failed=0`,
  `readback_failed=5`, `restore_failed=0`, `skipped=11`, `unsupported=1`,
  `bit_blocks_passed=0`, `bit_blocks_failed=0`
- UDP `1027`: same result as TCP.
- Bit-block checks are not run because LCPU does not support the block route.
...
- `SB6FF`: expected `Bool(true)`, observed `Bool(false)`
- `SB7FF`: expected `Bool(true)`, observed `Bool(false)`
- `SW6FF`: expected written `U16`, observed `U16(0)`
- `SW7FF`: expected written `U16`, observed `U16(0)`
- `SD1279`: expected written `U16`, observed `U16(0)`

Human review result:

- These readback NG points are expected PLC/system-device behavior for this
  target.
- They are recorded as `readback_failed` in the harness output so the raw test
  fact remains visible, but they are not treated as unresolved library bugs.
