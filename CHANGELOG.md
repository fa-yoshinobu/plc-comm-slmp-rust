...
- Release: Bumped the main crate and lockfile metadata to `5.1.1` for the additive high-level API and profile-limit release; the non-published `slmp-node` workspace crate remains `1.0.3`.
...
- Tests: Kept the response-correlation helper on the public `SlmpError` contract while explicitly allowing Rust 1.98 Clippy's `result_large_err` lint for that test-only helper.
