...
- Release: Bumped the main crate and lockfile metadata to `5.1.2` for the additive high-level API and profile-limit release; the non-published `slmp-node` workspace crate remains `1.0.3`.
...
- Tests: Kept the response-correlation test helper and shared operational-example helper on the public `SlmpError` contract while applying reasoned function-scoped allowances for Rust 1.98 Clippy's `result_large_err` lint, making the all-target Clippy gate compatible without changing the public error type.
